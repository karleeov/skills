...
- **Task** (HITL or AFK): Manual work that must happen before a *decision* can be made — nothing to decide, prototype, or research, but the discussion is blocked until it's done. Signing up for a service so its API can be judged, provisioning access, moving data so its shape can be seen. This is the one type that *does* rather than decides — and it earns its place by unblocking a decision, not by delivering the destination. The agent drives it alone where it can (AFK); otherwise it hands the human a precise checklist (HITL). **Any Task that spends money, creates accounts, provisions production access, or mutates data requires explicit user approval before execution — present the action, cost, and destination, and wait for confirmation.** Record only secret-manager references or credential identifiers in the resolution — never credentials themselves. Resolved when the work is done; the answer records what was done and any resulting facts (credential identifier, new URLs, row counts) later tickets depend on.
